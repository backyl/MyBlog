...
const APLAYER_TAG_REGEX =
  /\<div.*class=\"aplayer aplayer-tag-marker\"(.|\n)*\<\/script\>/g;
const FANCYBOX_TAG_REGEX =
  /\<div.*galleryFlag(.|\n)*\<\/span\>\<\/div\>\<\/div\>/g;
...
function buildTabNavAndContent(matches, tabName, tabActive) {
  ...
  for (let i = 0; i < matches.length; i += 2) {
    ...
    const tabHref = (tabName + " " + (i / 2 + 1))
      .toLowerCase()
      .split(" ")
      .join("-");

    const { content: contentWithAplayerTag, aplayerTag } =
      processAplayerTag(postContent);
    const { content: contentWithFancyboxTag, fancyboxTag } = processFancyboxTag(
      contentWithAplayerTag,
    );

    const renderedContent = hexo.render
      .renderSync({ text: contentWithFancyboxTag, engine: "markdown" })
      .trim();

    const finalContent = renderedContent
      .replace(/\<pre\>\<code\>.*@aplayerTag@.*\<\/code><\/pre>/, aplayerTag)
      .replace(/.*@fancyboxTag@.*/, fancyboxTag);

    const isActive =
      (tabActive > 0 && tabActive === i / 2 + 1) || (tabActive === 0 && i === 0)
        ? " active"
        : "";
